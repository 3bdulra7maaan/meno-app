...
function renderBanners(){$('banners-list').innerHTML=bannersCache.map((b)=>`<article class="question"><span class="status">${b.enabled?'مفعّل':'معطّل'} · ${esc(bannerTypeLabels[b.type])}</span><h2>${esc(b.title)}</h2><p>${esc(b.short_text)}</p><div class="meta">ترتيب ${b.display_order} · ${b.start_at?new Date(b.start_at).toLocaleString('ar'):'بداية فورية'} — ${b.end_at?new Date(b.end_at).toLocaleString('ar'):'بلا نهاية'}</div><div class="actions"><button data-banner-edit="${b.id}" class="quiet">تعديل</button><button data-banner-toggle="${b.id}" data-enabled="${!b.enabled}">${b.enabled?'تعطيل':'تفعيل'}</button></div></article>`).join('')||'<p class="meta">لا توجد بنرات. الصفحة الرئيسية ستعمل بصورة طبيعية.</p>';document.querySelectorAll('[data-banner-edit]').forEach((x)=>x.onclick=()=>editBanner(x.dataset.bannerEdit));document.querySelectorAll('[data-banner-toggle]').forEach((x)=>x.onclick=()=>toggleBanner(x.dataset.bannerToggle,x.dataset.enabled==='true'))}
function resetBannerForm(){ $('banner-form').reset();$('banner-id').value='';$('banner-order').value='0';$('banner-enabled').checked=true;$('banner-form').hidden=false;previewBanner() }
...
function editBanner(id){const b=bannersCache.find((x)=>x.id===id);$('banner-id').value=b.id;$('banner-title').value=b.title;$('banner-text').value=b.short_text;$('banner-image').value=b.image_url;$('banner-target').value=b.target_url||'';$('banner-type').value=b.type;$('banner-order').value=b.display_order;$('banner-start').value=localDate(b.start_at);$('banner-end').value=localDate(b.end_at);$('banner-enabled').checked=b.enabled;$('banner-form').hidden=false;previewBanner();$('banner-form').scrollIntoView({behavior:'smooth'})}
function previewBanner(){$('banner-preview').innerHTML=$('banner-image').value?`<img src="${esc($('banner-image').value)}" alt=""><div><b>${esc($('banner-title').value||'معاينة البنر')}</b><p>${esc($('banner-text').value)}</p></div>`:''}
$('new-banner').onclick=resetBannerForm;$('cancel-banner').onclick=()=>$('banner-form').hidden=true;['banner-title','banner-text','banner-image'].forEach((id)=>$(id).oninput=previewBanner);
$('banner-form').onsubmit=async(e)=>{e.preventDefault();const id=$('banner-id').value;const payload={title:$('banner-title').value.trim(),short_text:$('banner-text').value.trim(),image_url:$('banner-image').value.trim(),target_url:$('banner-target').value.trim()||null,type:$('banner-type').value,display_order:Number($('banner-order').value),start_at:$('banner-start').value?new Date($('banner-start').value).toISOString():null,end_at:$('banner-end').value?new Date($('banner-end').value).toISOString():null,enabled:$('banner-enabled').checked};const request=id?client.from('home_banners').update(payload).eq('id',id):client.from('home_banners').insert(payload);const {error}=await request;if(error){toast('تعذر حفظ البنر');return}$('banner-form').hidden=true;toast('تم حفظ البنر');loadBanners()};
